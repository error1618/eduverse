...
class CourseController extends Controller
{
    ...
    public function store(Request $request)
    {
       ...
       if ($validatedData->fails() ) {
        ...
        if ($request->filled('videoLink') && is_array($request->videoLink) && count(array_unique($request->videoLink)) > 0) {
            foreach (array_unique($request->videoLink) as $link) {
                if($link){
                DB::table('courses_videos')->insert([
                    'course_id' => $course->id,
                    'video' => $link,
                ]);
            }
    ...
    public function coursedetails($id)
{
    ...
    $videos = DB::table('courses_videos')->where('course_id', $id)
    ->get();    return view('dashboard.courses.coursedetails', compact('course','videos'));
}
