...
function handleFileUpload(files) {
  ...
  try{
    const maxLinks = 50; // Maximum number of video links allowed
    const videoFields = document.getElementById('videoFields');
    const addVideoButton = document.getElementById('addVideo');
    
    addVideoButton.addEventListener('click', () => {
        const videoCount = document.querySelectorAll('.video-input').length/2;
    
        if (videoCount < maxLinks) {
            const newVideoInput = document.createElement('div');
            newVideoInput.classList.add('video-input');
            newVideoInput.innerHTML = `
                <label for="videoLink">Video Link ${videoCount+1}:</label>
                <input type="text"  class="form-control video-input " name="videoLink[]" placeholder="Enter Video Link">
            `;
            videoFields.appendChild(newVideoInput);
        } else {
            alert('You have reached the maximum limit of video links (50).');
        }
    });
  }catch(e){
